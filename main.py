...
def criar_arestas():
    """ Processa a lista de objetos da classe Ponto e a converte para uma lista de
        objetos da classe Aresta """
...
def preencher_poligono_scanline(et: EdgeTable):
    """ Executa o algoritmo de preenchimento de polígonos segundo a coerência de arestas.
        As estruturas utilizadas são: ET e AET """
    AET = []
    ...
    while AET or any(et.linhas_de_varredura[y:] if y < TAMANHO_TELA else []):
        ...
        # PASSO 1: Remover arestas que terminam na linha ATUAL.
        # Esta operação acontece ANTES de adicionar novas arestas.
        AET = [aresta for aresta in AET if int(round(aresta.y_max)) > y]

        # PASSO 2: Adicionar arestas da ET que começam na linha ATUAL.
...
def mouse_click(botao, estado, x, y):
    ...
    if botao == GLUT.GLUT_RIGHT_BUTTON and estado == GLUT.GLUT_DOWN:
        if coletando_pontos and n_pontos >= 3:
            ...
            print("Fim da coleta de pontos. \nPolígono pronto para preenchimento.")
...
def main():
    ...
    GLUT.glutCreateWindow(b"Preenchimento de Poligonos - Coerencia de Arestas")
